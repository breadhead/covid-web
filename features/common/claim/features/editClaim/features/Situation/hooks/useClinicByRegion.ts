...
export const useClinicByRegion = (formContext: any, treatment: string) => {
  ...
  const currentRegion = getValueFromFormContext(
    formContext,
    treatment,
    'region',
  )
  const currentClinicValue = getValueFromFormContext(
    formContext,
    treatment,
    'clinic',
  )
  ...
  useEffect(
    () => {
      if (!!currentRegion) {
        api
          .searchClinicByRegion(currentRegion, currentClinicValue)
          .then(setRegionClinics)
      }
    },
