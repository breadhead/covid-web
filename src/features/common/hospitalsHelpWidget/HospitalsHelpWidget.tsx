...
const hospitals = (count) => declOfNum(count, ['больница', 'больницы', 'больниц']);
const toHospitals = (count) => declOfNum(count, ['больнице', 'больницам', 'больницам']);
    ...
    {!!data && (
      <div className={cx(styles.popup, {[styles.short]: short})}>
        <div className={cx(styles.half, styles.extended)}>
          ...
          <p className={styles.text}>{data.helpedCount} {toHospitals(data.helpedCount)}<br/>
          на {new Intl.NumberFormat(['ru-RU']).format(data.moneyGathered)} ₽ </p>
        </div>
        <div className={styles.half}>
          <span className={styles.label}>Ждут нашей помощи ещё</span>
          <p className={styles.text}> {data.helpWanted} {hospitals(data.helpWanted)}</p>
          <NavLink
